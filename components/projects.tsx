...
const projects = [
  { title: "CMS Wareg Tasik", category: "Infrastructure", tech: ["Laravel", "Filament"], desc: "Manajemen konten terpusat untuk platform kuliner Tasikmalaya.", img: "https://images.unsplash.com/photo-1555066931-4365d14bab8c?q=80&w=2070" },
  ...
  { title: "POS Warung", category: "Management", tech: ["Laravel", "Alpine.js"], desc: "Aplikasi Point of Sale terintegrasi untuk efisiensi UMKM.", img: "https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?q=80&w=2070" }
];
  ...
  <section id="projects" className="py-24 px-8 bg-[#FAF9F6]">
    <div className="max-w-[1400px] mx-auto">
      <header className="flex justify-between items-end mb-16 border-b border-black/10 pb-8">
        <h2 className="text-5xl md:text-7xl font-black uppercase italic tracking-tighter leading-none text-[#1A1A1A]">Projects.</h2>
        <p className="text-[10px] font-bold text-[#0055FF] uppercase tracking-[0.4em] hidden md:block">Case Studies / 2026</p>
      </header>
        ...
        {projects.map((proj, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="group bg-white border border-black/5 p-6 hover:shadow-2xl transition-all duration-500 hover:-translate-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {proj.tech.map(t => (
                <span key={t} className="text-[9px] font-black uppercase tracking-widest text-slate-400 border border-slate-200 px-2 py-1">{t}</span>
              ))}
            </div>
          </motion.div>
        ))}
